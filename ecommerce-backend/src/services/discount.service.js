...
class DiscountService {
  ...
  static async getDiscountAmount({ codeId, userId, shopId, products }) {
    ...
    if (discount_max_uses_per_user > 0) {
      //  co the dung 1 hoac nhieu lan
      const userDiscount = discount_users_used.find(
        (user) => user.userId === userId
      );
      if (userDiscount) {
        throw new BusinessLogicError(
          `The number of discount codes for 1 person is ${discount_max_uses_per_user}`
        );
      }
    }
    ...
    // kiem tra sp co nam trong danh sach giam gia cua shop ko

    // thêm vào user_used và chỉnh giống như ki cancel
    return {
      totalOrder,
      discount: amount,
